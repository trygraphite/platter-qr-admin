...
const CreateTable = () => {
  ...
  const [createdTable, setCreatedTable] = useState<{ name: string; link: string } | null>(null)
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      const tableUrl = generateTableQRUrl(
        ...
        process.env.NEXT_PUBLIC_BASE_DOMAIN || 'https://platterng.com/',
        table.link
      )
      
      console.log('tableUrl:', tableUrl)
      console.log('primaryBusiness?.name:', primaryBusiness?.name)
      ...
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.GET_ALL_BUSINESS_TABLES] })
    },
    onError: () => {
    ...
    <>
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="max-w-md mx-auto bg-transparent space-y-6 p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <span className='text-white'>Create a new table for your restaurant. Tables help organize seating and orders.</span>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        <div className="space-y-2">
          ...
          <Input id="name" {...register('name')} placeholder="e.g. Table 1, VIP Table, Outdoor 1" />
          ...
          <div className="flex justify-center">
            ...
            {qrCodeUrl && createdTable && (
              <div className="text-xs text-muted-foreground mt-2">
                Debug: tableName={createdTable.name}, restaurantName={primaryBusiness?.name || "The Sauce"}
              </div>
            )}
          </div>
        </DialogContent>
