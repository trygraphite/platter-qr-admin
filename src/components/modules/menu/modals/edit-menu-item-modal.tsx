...
export function EditMenuItemModal({ isOpen, onClose, menuItem, categoryId, groupId }: EditMenuItemModalProps) {
  ...
  const [selectedServicePoint, setSelectedServicePoint] = useState<string>(menuItem.servicePoint || "")
  ...
  useEffect(() => {
    if (menuItem.varieties && menuItem.varieties.length > 0) {
      const convertedVarieties: MenuItemVariety[] = menuItem.varieties.map((variety) => ({
        name: variety.name,
                ...
                {varieties.map((variety, index) => (
                  <div key={index} className="border rounded-lg p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        <Label htmlFor={`variety-description-${index}`}>Description</Label>
                        <Input
